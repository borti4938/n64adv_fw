...
- Advanced features:
  ...
  - line doubling of 240p/288p video to 480p/576p
  - line trippling of 240p to (pseudo) 720p (NTSC content only, generated resolution is not a standard resolution so compatibility highly depends on your TV)
...
\[2\] Not available if the filter addd on is used as HSYNC and VSYNC are shared outputs with F1 and F2 (filter selection), however VGA with CSYNC remains intact  
...
The menu always opens in the top level, where you have following options available:
...
You can switch between them and the miscellaneous settings by either using the respective menu entry or by pressing **L** or **R**.

Pressing B in the top menu closes the OSD.
Alternatively, you can use **D-le + L + R + C-le** to close the menu at any point.
If you feel to just undisplay the OSD for a moment to check your last settings change, just hold **L + Z**.
...
| **Video - Color depth** | Shows whether the output is generated in 21bit mode or 16bit mode |
...
\[9\] If this is _Off_, the other scanline options won't be accessible.  
...
| **- Field-Shift Fix** | Off | Shifts all fields half of a pixel up and down between line doubled even and odd field reducing some typicall up-down-flickering of simple bob deinterlacing in most cases |
...
| Entry | Default | Description |
|:------|:--------|:------------|
| **Settings for:** \[14\] | _Current_ | Selects configuration parameters between different modes (mode depends on NTSC/PAL, progressive/interlaced and LineX) |
...
| **Load - N64 Standard** | Load N64 standard setting, e.g. turn off line multiplier.<br>These settings are also loaded if you hold down reset button while power cycle the system. |
...
The welcome screen is determined by compile time of the software.
...
- Download the latest firmware from the [GitHub Repository](https://github.com/borti4938/n64adv_fw/releases)
...
Please note that the **console must be turned on** to provide reference target voltage for the programming device.
...
Now you are using the pre-build QSYS-IPs during synthesis.
This method speeds up compilation time a lot.
